...
def get_points_by_user(db: Session, user_id: int):
    return db.query(models.PointOfInterest).filter(models.PointOfInterest.user_id == user_id).all()

def delete_point(db: Session, point_id: int, user_id: int):
    db_point = db.query(models.PointOfInterest).filter(models.PointOfInterest.id == point_id, models.PointOfInterest.user_id == user_id).first()
    if db_point:
        db.delete(db_point)
        db.commit()
    return db_point
